...
const FlowDiagram = ({ activeScenario }: { activeScenario: string }) => {
  ...
  useEffect(() => {
    const updatedNodes = (computedNodes as Node[]).map(node => ({
        ...
        style: node.style || { width: NODE_WIDTH, height: NODE_HEIGHT, background: 'defaultColor', border: 'defaultBorder', padding: 10, borderRadius: 5, fontSize: 12, textAlign: 'center', display: 'flex', justifyContent: 'center', alignItems: 'center' }
    }));
...
export default function ConfigureAIAgentPage({ params }: { params: Promise<{ useCase: string }> }) {
