...
import { getUseCaseData } from "@/lib/use-case-data"

export default function UseCasePage({ params }: { params: { industry: string; useCase: string } }) {
  const industry = params.industry.toLowerCase()
  const useCase = params.useCase.toLowerCase()

  const useCaseData = getUseCaseData(industry, useCase)
